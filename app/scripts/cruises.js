...
System.import(`../data/splash/index.${language}.js`).then((res) => {
	try {
		...
		content.innerHTML = createLayout(res.default[dataIndex]) + createModal(res.default[dataIndex])
		setPresentation(res.default[dataIndex].media);
		addListeners();
	} catch(err) {
		//
		console.log(err)
...
function createLayout(data) {
	return `
		<div class="cuise-body">
			<div class="cruise-data">
				...
				<div class="reservation-btn">
					<button class="bttn-unite bttn-md bttn-warning" data-target="#${data.id}-modal" data-toggle="modal">${ reserve }</button>
				</div>
			</div>
		...
		<div class="cruise-itinerary">
			...
			<div class="items">
				<h3>${ itinerary }</h3>
				${ getItineraryItems(data.itinerary) }
			</div>
		</div>
	`;
}
...
function setPresentation(media) {
	...
	setInterval(() => {
		...
		document.querySelector('.img-item:not(.hidden)').classList.add('hidden');
		document.querySelectorAll('.img-item')[i].classList.remove('hidden');
	}, 10000)
}
...
function getItineraryItems(itinerary) {
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title">${ item.title + ' ' +item.subtitle }</h3>
				<div class="price-holder"><span class="form_price">${item.price.amount}$</span><span>${ t0 }</span></div>
			</div>
