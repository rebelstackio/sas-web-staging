...
function newDraft($session) {
    ...
    $log ->LogDebug("session passed to newDraft: ".$session);
    try {
      ...
      $log->LogDebug("session malformed: ".json_encode($session));    
      throw new BadRequestException("session parameter (json) is malformed: ".$e->getMessage());
    }

    try {
        require_once("dbconn.php");
        $log->LogDebug("check existing session id: ".$sessionID);    
        if (getReservation($clientEmail,$sessionID)) {// correct paramaters?
          $log->LogDebug("error: reservationid already exists ".$sessionID);    
          throw new BadRequestException('Create Reservation Error: A Reservation with the same ID already exists.');
        } else {
          $log->LogDebug("no existing reservationid - keep going");
        }
    } catch (Exception $e) {
      $log->LogDebug("we're suppossed to be loggging something here... there is a reservation with same ID already"); 
      throw $e;
    }

    
    try {
      $log->LogDebug("parse merchant_info");    
