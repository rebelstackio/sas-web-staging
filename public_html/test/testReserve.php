...
*** RESERVATION STATES & NOTIFICATIONS
       DRAFT | DRAFT NOTICE        (sent on first item)                <== Handled by this program
       DRAFT | DRAFT REMINDER      (sent 1 day after creation)         <== CRON
update DRAFT | DRAFT UPDATE NOTICE (send to client and responder)      <== Handled by this program
    DRAFTFIN | DRAFT FINAL NOTICE  (sent on submit request)            <== Handled by this program
    DRAFTACC | DRAFT ACCEPTED      (sent on confirmed) !operator       <== Handled by operator program
        SENT | RESERVE REQUEST     (sent on confirmed) !operator       <== Handled by operator program
       SENT2 | RESERVE REMINDER 1  (sent 14 -> 3 days before due_date  <== CRON
       SENT3 | RESERVE REMINDER 2  (sent  2 -> 0 days before due_date  <== CRON
        PAID | VOUCHER-INFO        (sent on payment)                   <== PAYMENT Handler
      CANCEL | CANCEL NOTICE       (sent on cancel)                    <== Handled by this program
      REFUND | REFUND              (send on refund) !operator          <== PAYMENT Handler


** Notes for CRON ==> DUE_DATE
   -minimum 3 days before activity
   -that means all reservation requests (drafts) must be made for minimum 3 days in advance of activty/tour date
   -unfinalized drafts (reservation requests not submitted) will have reminders sent 1 day after creation


//throw new Exception("error mesage");
...
$smtpuser = "rein.petersen@gmail.com" // "info@southamericanssecrets.com";
$smtppass = "dont commit passwords to git! use .env!!!";
